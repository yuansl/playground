...
func (srv *Server) HandleVoteRequest(ctx context.Context, req *VoteRequest) (*VoteResponse, error) {
	...
	if srv.state == FOLLOWER_STATE && req.Term >= srv.Term && srv.Leader == "" {
		if srv.voteFor == "" {
			resp.Term = req.Term
			srv.Term = req.Term
			srv.voteFor = req.From.Addr
			srv.election.Reset(electionTimeout())
		} else if srv.voteFor == req.From.Addr {
			srv.voteFor = ""
			srv.Leader = req.From.Addr
			srv.election.Stop()
		}
	}
	return &resp, nil
...
func main() {
	server := Server{Addr: "A", Others: []*Node{{Addr: "B"}, {Addr: "C"}}, Term: 0, election: time.NewTimer(electionTimeout())}
